public class CDLinkedList {
	...
	public void swapRange(Iterator s1,Iterator f1,Iterator s2,Iterator f2) {
		if(s1 == null || f1 == null || s2 == null || f2 ==null) {                          //if the range is null then return
			return;
		}
		
		if(!(s1 instanceof DListIterator && f1 instanceof DListIterator && s2 instanceof DListIterator && f2 instanceof DListIterator)) {
			return;
		}
		DListIterator start1 = (DListIterator) s1;                                         //cast the iterator to be DListIterator
		DListIterator final1 = (DListIterator) f1;
		DListIterator start2 = (DListIterator) s2;
		DListIterator final2 = (DListIterator) f2;
		
		DListIterator bs1 = new DListIterator(start1.currentNode.previousNode);            //create iterator that is the front of both range
		DListIterator bs2 = new DListIterator(start2.currentNode.previousNode);
		
		bs1.currentNode.nextNode = start2.currentNode;                                     //swap the next pointer that point in to node s1,s2
		bs2.currentNode.nextNode = start1.currentNode;
		
		DListIterator af1 = new DListIterator(final1.currentNode.nextNode);                //create iterator that is the back of both range
		DListIterator af2 = new DListIterator(final2.currentNode.nextNode);
		
		af1.currentNode.previousNode = start2.currentNode;                                 //swap the previous pointer that point in to f1,f2
		af2.currentNode.previousNode = start1.currentNode;
		
		start1.currentNode.previousNode = bs2.currentNode;                                 //swap the previous pointer pointing out of the range
		start2.currentNode.previousNode = bs1.currentNode;
		
		final1.currentNode.nextNode = af2.currentNode;                                     //swap the next pointer pointing out of the range
		final2.currentNode.nextNode = af1.currentNode;
	}

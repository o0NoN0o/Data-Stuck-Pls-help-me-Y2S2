...
public class QueueArray implements MyQueue {
	...
	public void reverse() throws EmptyQueueException {
		if(isEmpty())
			return;
		int temp;
		for (int i = 0; i < (size)/2; i++) {                                                    //run through the list and swap the front and back using index
			temp = theArray[front+i];                                                           //only able to do in array queue
			theArray[front+i] = theArray[(front + (size-1-i))%theArray.length];
			theArray[(front + (size-1-i))%theArray.length] = temp;
		}

...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		//TODO
		//FILL CODE
		if(subtree.isEmpty()) {        //check if the subtree is empty
			return;
		}
		if(this.isEmpty()) {            //check if the tree is empty
			this.root = subtree.root;   //change the root of our tree to subtree
			this.size = subtree.size;   //change the size of our tree to subtree
		}
		TreeIterator min = (TreeIterator) findMin();                 //find the min node to add the subtree to
		TreeIterator tmax = (TreeIterator) subtree.findMax();        //find the max node of the subtree
		TreeIterator t = new TreeIterator(subtree.root);             //root of subtree
		if(tmax.currentNode.data > min.currentNode.data) {           //check if it will still be BST of the max of subtree is greater
			return;                                                  // than the min of our tree then not BST
		}
		min.currentNode.left = t.currentNode;                        //add the subtree
		this.size += subtree.size;                                   //add the size
